...
class Benchmark:
    ...
    def generateRcSScript(self, rcSFilename, simsize, threads):
        ...
        runscript_file.write('mount -t ext3 /dev/hdb benchmarks\n')
    ...
    def runBenchmark(self, gem5FusionRoot, gem5Bin, fullsystem = False, simsize = None, gem5Params = '', configParams = '', threads = None, runscript = None, revisionData = None):
        ...
        if revisionData:
            import json
            with open(os.path.join(outputDir, "run.rev"), 'w') as out:
                json.dump(revisionData, out)
        ...
        if fullsystem:
            ...
            if runscript is None:
                if simsize != 'default':
                    if threads:
                        ...
                        rcSScript = os.path.join(self.rcSDir, '%s_%s_%s.rcS' % (self.suite, self.name, simsize))
                else:
            ...
            command = '%s --outdir=%s %s %s %s --script=%s' % (gem5BinPath, '.', gem5Params, configScript, configParams, rcSScript)
        ...
        # Handle syscall emulation mode
            ...
            # Setup the command to run
            command = '%s --outdir=%s %s %s %s -c %s -o "%s" --output=%s ' % (gem5BinPath, '.', gem5Params, configScript, configParams, benchmarkBinPath, benchmarkCmdLine, outFile)
        ...
        command = ("cd %s; " % outputDir) + command + "; cd -"
