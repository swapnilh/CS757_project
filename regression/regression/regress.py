...
parser.add_option("--gdb", action="store_true", default=False, help="run in GDB")
...
gem5FusionRoot = os.path.abspath(options.gem5_fusion_root)
...
if not options.debug:
    revInfo = revisions.retrieve(gem5FusionRoot, True)
else:
    revInfo = ''
for bench in benchmarks:
    ...
    if options.restore_from_checkpoint:
        ...
        if not options.checkpoint_base_dir and not options.checkpoint_dir:
